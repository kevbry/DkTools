    ...
    public class AppRepo : IAppRepo
    {
        ...
        public AppRepo(DkAppSettings appSettings)
        {
            ...
            try
            {
                ...
                {
                    if (!appSettings.FileSystem.DirectoryExists(repoDir)) appSettings.FileSystem.CreateDirectory(repoDir);

                    _repoDir = PathUtil.CombinePath(repoDir, RepoBaseDir);
                    if (!appSettings.FileSystem.DirectoryExists(_repoDir)) appSettings.FileSystem.CreateDirectory(_repoDir);

                    _repoFileName = PathUtil.CombinePath(_repoDir, AppNameEncode(_appSettings.AppName));
                    if (appSettings.FileSystem.FileExists(_repoFileName))
                    {
        ...
        private void PurgeNonexistentFiles()
        {
            IterateFiles(file =>
            {
                if (!_appSettings.FileSystem.FileExists(File_GetFileName(file)))
                {
                    ClearFileNode(file);
                }
...
//#endif
        ...
        private void Write()
        {
            ...
            byte[] fileData;
            using (var memStream = new IO.MemoryStream())
            using (var writer = new IO.BinaryWriter(memStream))
            {
            ...
            _appSettings.FileSystem.WriteFileBytes(_repoFileName, fileData);
        }
        ...
        private void Read()
        {
            ...
            using (var rdr = new IO.BinaryReader(memStream))
            {
            ...
            using (var memStream = new IO.MemoryStream(fileData))
            using (var rdr = new IO.BinaryReader(memStream))
            {
