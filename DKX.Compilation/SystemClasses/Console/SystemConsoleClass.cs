...
namespace DKX.Compilation.SystemClasses.Console
{
    class SystemConsoleClass : SystemClass
    {
        ...
        private CodeFragment Generate_WriteLine(CodeGenerationContext context, Chain[] arguments, Span span)
        {
            if (arguments.Length != 1) throw new CodeException(span, ErrorCode.InvalidNumberOfArguments);
            var argFrag = arguments[0].ToWbdkCode_Read(context);
            Conversions.ConversionValidator.CheckConversion(DataType.String255, argFrag, context.Report);
